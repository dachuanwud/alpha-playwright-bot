"""
启动 abin 账号的 Chrome 浏览器（端口 9223）
"""
import subprocess
import time
import os
...
def start_chrome_window():
    ...
    # 获取 abin 账号配置
    config = get_account_config('abin')
    if not config:
        print("❌ 未找到 abin 账号配置")
        return False
    ...
    # 先尝试关闭现有进程
    print("1. 检查并关闭现有 Chrome 进程（如果存在）...")
        ...
        for i in range(10):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print("\n" + "=" * 60)
                    print("✅ abin 账号的浏览器窗口应该已经打开")
                    print("=" * 60)
                    print("\n📋 接下来的步骤：")
                    print("   1. 检查浏览器窗口是否已打开")
                    print("   2. 如果未登录，请登录 abin 账号")
                    print("   3. 切换到要交易的代币页面")
                    print("   4. 运行: python main.py --account abin")
                    return True
            except:
                print(f"   等待中... ({i+1}/10)")
